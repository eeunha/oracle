...
SELECT * FROM tblmemo WHERE name = '홍길동'; -- 하나만 보고싶은데 두개가 나온다. 식별자가 없기 때문.
SELECT * FROM tblmemo WHERE seq = 1;

DELETE FROM tblmemo WHERE seq = 2;
DELETE FROM tblmemo WHERE seq = 1; -- 중복되는 값도 모두 제거됨. 문제 발생
...
(
	...
	regdate DATE,				-- 작성날짜
	-- 중요도(1(중요), 2(보통), 3(안중요))
--	priority NUMBER CHECK (priority >= 1 AND priority <= 3)	
...
-- ORA-02290: check constraint (HR.SYS_C007097) violated
INSERT INTO tblmemo (seq, name, memo, regdate, priority, category) 
