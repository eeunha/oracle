...
/*
 	...
 	1. to_char(숫자 [, 형식문자열])		: 숫자 > 문자
 	
 	형식 문자열 구성요소
 	a. 9: 숫자 1개를 문자 1개로 바꾸는 역할. 빈 자리를 스페이스로 치환. > %5d
 	b. 0: 숫자 1개를 문자 1개로 바꾸는 역할. 빈 자리를 0으로 치환. > %05d
 	c. $: 통화 기호 표현
 	d. L: 통화 기호 표현(Locale)
 	e. .: 소숫점
 	f. ,: 천단위 표기
...
SELECT
	weight,
	'@' || to_char(weight) || '@',
	'@' || to_char(weight, '99999') || '@',  -- @    64@  공백 4. 공백 중 하나는 부호의 자리다. +는 안보이고 -는 보인다.
	'@' || to_char(-weight, '99999') || '@', -- @   -64@
	'@' || to_char(weight, '00000') || '@',  -- @ 00064@  공백 4. 공백 중 하나는 부호의 자리다. +는 안보이고 -는 보인다.
...
SELECT
	...
	to_char(2341234567.89, '9,999,999.9') -- 너무 길면 값이 제대로 나오지 않는다.
FROM dual;
...
SELECT to_char(sysdate, 'dd') FROM dual;	--일(이번 달의 며칠)
SELECT to_char(sysdate, 'd') FROM dual;		--일(이번 주의 며칠) == 요일(숫자)
...
SELECT
	...
	to_char(sysdate, 'day am hh:mi:ss') --화요일 오후 02:48:13
FROM dual;
...
FROM
	tblinsa
WHERE 
	ibsadate >= '2000-01-01'; -- 문자열 > 암시적 형변환
...
FROM
	tblinsa
WHERE 
	ibsadate >= '2000-01-01' AND ibsadate <= '2000-12-31'; -- 오답
...
FROM
	tblinsa
WHERE 
	ibsadate >= '2000-01-01 00:00:00' AND ibsadate <= '2000-12-31 23:59:59'; -- 오답	
...
SELECT
	...
	to_date('2023-08-29 15:28:39', 'yyyy-mm-dd hh24:mi:ss') -- 반드시 뒤에 형식문자를 붙여주어야 한다.
FROM
	dual;
...
FROM
	tblinsa
WHERE
	ibsadate >= to_date('2000-01-01 00:00:00', 'yyyy-mm-dd hh24:mi:ss')
	AND ibsadate >= to_date('2001-12-31 23:59:59', 'yyyy-mm-dd hh24:mi:ss');