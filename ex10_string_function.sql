...
-- 이름(first_name)에 'an' 포함된 직원 > 대소문자 구분 없이
SELECT
	first_name
FROM
	employees
WHERE
	first_name LIKE '%an%' OR first_name LIKE '%AN%' OR first_name LIKE '%An%' OR first_name LIKE '%aN%';
...
FROM
	tblcountry
WHERE leng > 3; -- ORA-00904: "LENG": invalid identifier
