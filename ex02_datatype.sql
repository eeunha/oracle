...
1. 숫자형
    ...
    a. number
        ...
        - 5~22byte
        - 1X10^-130 ~ 9.9999X10^125 -> 모든 숫자 표현 가능하다~
...
2. 문자형
    ...
    b. nchar
        - n: national > 오라클 인코딩과 상관없이 해당 컬럼을 UTF-16으로 동작하게 한다. 
...
4. 이진 데이터형
    ...
    - 잘 사용 안함.
    ex) 게시판(첨부파일), 화원가입(사진) > 파일명만 저장(문자열)
    a. blob
        - 최대 128TB
...
--    num number(4, 2) -- -99.99 ~ 99.99  (정수자리수+실수자리수, 소숫점뒤자리수)

--    txt char(10) -- 최대 10바이트까지의 문자열. byte 키워드 생략
...
insert into tblType (num) values (3.99); -- 반올림 한 정수로 들어감
...
-- 데이터 가져오기 > 결과 테이블(Result Table), 결과셋(Result Set)
select * from tblType;

commit;

-- *** 오라클은 모든 식별자를 대문자로 생성한다.
