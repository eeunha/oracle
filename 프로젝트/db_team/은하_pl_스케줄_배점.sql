...
as
select 
    sl.subseq as 과목번호, sch.teacherseq as 교사번호, sl.courseseq as 과정번호, c.coursename as 과정명, c.coursestartdate as 과정시작일, 
    c.coursefinishdate as 과정종료일, c.classroomname as 강의실, s.subname as 과목명, sl.subjectstartdate as 과목시작일, 
    sl.subjectfinishdate as 과목종료일, b.bookname as 교재명, c.studentnumber as 교육생등록인원,
    case
...
exception
    ...
    when others then
        dbms_output.put_line('예외 처리');
end procSetPrctTestReg;


-- 실행
begin
--    procSetPrctTestReg(1, 3); -- 이미 등록
