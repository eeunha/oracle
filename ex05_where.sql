...
-- 3. 부서가 '개발부'이고, 급여(basicpay)가 150만원 이상 받는 모든 직원을 가져오시오.
SELECT * FROM TBLINSA WHERE BUSEO = '개발부' AND BASICPAY >= 1500000;

-- 4. 급여(basicpay) + 수당(sudang)을 합한 금액이 200만원 이상 받는 모든 직원을 가져오시오 
...
SELECT * FROM TBLINSA 
WHERE 1 = 1; -- 조건절에 반드시 컬럼이 포함되지 않아도 된다.   60명 전원 다 통과 > 있으나마나
...
SELECT * FROM TBLINSA WHERE BASICPAY BETWEEN 1020000 AND 1200000;
...
-- 2. 문자형(문자코드)
SELECT * FROM TBLINSA WHERE NAME > '이순신'; -- name.compareTo("이순신")  -> 이순신보다 뒤에 있는 사람 가져오기
...
SELECT * FROM TBLINSA WHERE NAME LIKE '%길%'; -- 이름에 길이 포함된 사람 찾기
